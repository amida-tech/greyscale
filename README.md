...
`docker run --name indaba-postgres -ti -e POSTGRES_PASSWORD=indabapassword -p 5432:5432 -v /Users/cporter/Amida/greyscale/backend/db_setup:/shared-volume -d postgres:9.6.5-alpine`
...
***
`-v /Users/cporter/Amida/greyscale/backend/db_setup:/shared-volume`
...
![Create PostgreSQL user](images/readme/step5_create_database.png)

6. Use `psql` to restore an indaba database (in the container, these files are located in `/shared-volume`, in the git repo, they are located in `/greyscale/backend/db_setup` ):

`psql -h localhost -U indabauser indaba < /shared-volume/schema.indaba.sql` 
![Beginning of Schema import](images/readme/step6.1_schema_import.png)
.
.
.
...
On the local machine, NOT the docker container, run
...
13. Check localhost:80. If you’re on a Mac, you may have a default Apache server running that you need to kill.
