...
describe(testTitle, function () {
    ...
    function allTests() {
        ...
        describe(level(++l1) + testTitle + 'Select empty (before testing) ', function () {
            it(level(l1, ++l2) + '(Err) taskId must be specified', function (done) {
                ithelper.selectErrMessage(testEnv.apiCreatedRealm, path, tokenSuperAdmin, 400, 403, 'taskId must be specified', done);
            });
            it(level(l1, ++l2) + '(Err) taskId must be integer', function (done) {
            ...
            it(level(l1, ++l2) + '(Err) taskId does not exist', function (done) {
                ithelper.selectErrMessage(testEnv.apiCreatedRealm, path + '?taskId=' + notExistTaskId, tokenSuperAdmin, 400, 403, 'does not exist', done);
            });
            it(level(l1, ++l2) + 'True number of records (superAdmin) = 0', function (done) {
                ithelper.selectCount(testEnv.apiCreatedRealm, path + '?taskId=' + taskId[0], tokenSuperAdmin, 200, 0, done);
            });
            it(level(l1, ++l2) + 'True number of records (admin) = 0', function (done) {
                ithelper.selectCount(testEnv.apiCreatedRealm, path + '?taskId=' + taskId[0], tokenAdmin, 200, 0, done);
            });
            it(level(l1, ++l2) + 'True number of records (user1) = 0', function (done) {
                ithelper.selectCount(testEnv.apiCreatedRealm, path + '?taskId=' + taskId[0], tokenUser1, 200, 0, done);
            });
        });
        describe(level(++l1) + testTitle + 'Add comment (not flagged) ', function () {
            l2=0;
            describe(level(l1, ++l2) + 'Errors:', function () {
                l3=0;
                it(level(l1, l2, ++l3) + '(Err) questionId must be specified', function (done) {
                    insertItem = {};
                ...
                it(level(l1, l2, ++l3) + level(l1, ++l2) + '(Err) questionId must be integer', function (done) {
                    insertItem = {
                ...
                it(level(l1, l2, ++l3) + '(Err) questionId does not exist', function (done) {
                    insertItem = {
                ...
                it(level(l1, l2, ++l3) + '(Err) taskId must be specified', function (done) {
                    insertItem = {
                ...
                it(level(l1, l2, ++l3) + '(Err) taskId must be integer', function (done) {
                    insertItem = {
                        questionId: questionId[0],
                        taskId: errTaskId
                    };
                ...
                it(level(l1, l2, ++l3) + '(Err) taskId does not exist', function (done) {
                    insertItem = {
                        questionId: questionId[0],
                ...
                it(level(l1, l2, ++l3) + '(Err) discussion`s entry must be specified', function (done) {
                    insertItem = {
                        questionId: questionId[0],
                        taskId: taskId[0],
                        stepId: stepId[1]
            ...
            describe(level(l1, ++l2) + 'Success:', function () {
                l3=0;
                it(level(l1, l2, ++l3) + 'Simple comment from User1 (Step1, Question1)', function (done) {
                    insertItem = {
                        questionId: questionId[0],
                        taskId: taskId[0],
                        stepId: stepId[0],
                ...
                it(level(l1, l2, ++l3) + 'Get entry update for added entry (User1 - true)', function (done) {
                ...
                it(level(l1, l2, ++l3) + 'Get entry update for added entry (User2 - false)', function (done) {
                ...
                it(level(l1, l2, ++l3) + 'Get entry update for added entry (Admin - true)', function (done) {
                ...
                it(level(l1, l2, ++l3) + 'Update Simple comment from Admin (Step1, Question1)', function (done) {
                    updateItem = {
                        questionId: questionId[0],
                        taskId: taskId[0],
                        stepId: stepId[0],
                ...
                it(level(l1, l2, ++l3) + 'Get comments content', function (done) {
                ...
                it(level(l1, l2, ++l3) + 'Simple comment from User1 (Step1, Question2)', function (done) {
                    insertItem2 = {
                ...
                it(level(l1, l2, ++l3) + '(Err) Delete created comment (User2 couldn`t delete comment)', function (done) {
                ...
                it(level(l1, l2, ++l3) + 'True number of discussion`s entries = 2', function (done) {
                    ithelper.selectCount(testEnv.apiCreatedRealm, path + '?taskId=' + taskId[0], tokenUser1, 200, 2, done);
                });
                it(level(l1, l2, ++l3) + 'Delete created comment (Admin has rights to delete)', function (done) {
                    ithelper.deleteOne(testEnv.apiCreatedRealm, path + '/' + insertItem2.id, tokenAdmin, 204, done);
                });
                it(level(l1, l2, ++l3) + 'True number of discussion`s entries = 1', function (done) {
                    ithelper.selectCount(testEnv.apiCreatedRealm, path + '?taskId=' + taskId[0], tokenUser1, 200, 1, done);
                });
                it(level(l1, l2, ++l3) + 'True number of discussion`s entries for task2 = 0', function (done) {
                    ithelper.selectCount(testEnv.apiCreatedRealm, path + '?taskId=' + taskId[1], tokenUser1, 200, 0, done);
                });
            });
        });
        describe(level(++l1) + testTitle + 'get Users ', function () {
            l2=0;
            it(level(l1, ++l2) + 'for task1 without blindReview flag - Admin request', function (done) {
