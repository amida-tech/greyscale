        ...
        describe(testTitle+'All of tests for admin `' + user.firstName+'`', function () {
            ...
            describe(testTitle+'Prepare for main tests - creating Roles and Rights ', function () {
                it('CRUD: Create new Role - "testRole" (non Sysytem)', function (done) {
                    ithelper.insertOne(testEnv.api_created_realm, '/roles', user.token, {name: 'testRole'}, 201, obj, 'roleTestId', done);
                });
                it('CRUD: Create new Role - "systemRole" (Sysytem)', function (done) {
                    ithelper.insertOne(testEnv.api_created_realm, '/roles', user.token, {name: 'systemRole', isSystem: true}, 201, obj, 'roleSystemId', done);
                });
                it('CRUD: Create new Right - "testRight"', function (done) {
                    ithelper.insertOne(testEnv.api_created_realm, '/rights', user.token, {action: 'testRight', description: 'test Right Description'}, 201, obj, 'rightTestId', done);
                });
            });
