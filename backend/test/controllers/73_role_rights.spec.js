    ...
    function userTests(user) {
        describe(testTitle + 'All of tests for user `' + user.firstName + '`', function () {
            ...
            it('(Err) No rights to select for ordinary users', function (done) {
                ithelper.getCheckRights(testEnv.apiCreatedRealm, '/roles/3/rights', user.token, 400, 401, 'User\'s role has not permission for this action(s)', done);
            });
        });
    }
        ...
        describe(testTitle + 'All of tests for admin `' + user.firstName + '`', function () {
            if (user.roleID === 1) {
                it('Select true number of records for SuperAdmin', function (done) {
                    ithelper.selectCount(testEnv.apiCreatedRealm, '/roles/1/rights', user.token, 200, 0, done);
                });
            } else if (user.roleID === 2) {
                it('Select true number of records for Admin', function (done) {
                    ithelper.selectCount(testEnv.apiCreatedRealm, '/roles/2/rights', user.token, 200, numberOfRecords, done);
                });
            }
            describe(testTitle + 'Prepare for main tests - creating Roles and Rights ', function () {
                it('CRUD: Create new Role - "testRole" (non Sysytem)', function (done) {
                    ithelper.insertOne(testEnv.apiCreatedRealm, '/roles', user.token, {
                        name: 'testRole'
                ...
                it('CRUD: Create new Right - "testRight"', function (done) {
                    ithelper.insertOne(testEnv.apiCreatedRealm, '/rights', user.token, {
                        action: 'testRight',
            ...
            describe(testTitle + 'Main tests ', function () {
                ...
                it('Create new Role-Right - SUCCESS', function (done) {
                    ithelper.insertOne(testEnv.apiCreatedRealm, '/roles/' + obj.roleSystemId + '/rights/' + obj.rightTestId, user.token, insertItem, 201, obj, 'id', done);
                });
                ...
                it('Select true number of records (after creating)', function (done) {
                    ithelper.selectCount(testEnv.apiCreatedRealm, '/roles/' + obj.roleSystemId + '/rights', user.token, 200, 1, done);
                });
                ...
                it('Delete created Role-Right', function (done) {
                    ithelper.deleteOne(testEnv.apiCreatedRealm, '/roles/' + obj.roleSystemId + '/rights/' + obj.rightTestId, user.token, 204, done);
                });
                it('Select true number of records (after deleting)', function (done) {
                    ithelper.selectCount(testEnv.apiCreatedRealm, '/roles/' + obj.roleSystemId + '/rights', user.token, 200, 0, done);
                });
            });
            describe(testTitle + 'Clean up after main tests - deleting Roles and Rights ', function () {
                it('Delete system role', function (done) {
                    ithelper.deleteOne(testEnv.apiCreatedRealm, '/roles/' + obj.roleSystemId, user.token, 204, done);
                });
                it('Delete test role', function (done) {
                    ithelper.deleteOne(testEnv.apiCreatedRealm, '/roles/' + obj.roleTestId, user.token, 204, done);
                });
                it('Delete test right', function (done) {
                    ithelper.deleteOne(testEnv.apiCreatedRealm, '/rights/' + obj.rightTestId, user.token, 204, done);
                });
            });
        });
    }
