...
angular.module('greyscaleApp')
    ...
    greyscaleSurveyAnswerApi.list({ questionId: params.field.id, taskId: $stateParams.taskId }).then(function (_answers) {
        ...
        for (var i = _answers.length - 1; i >= 0; i--) {
            if (_answers[i].version === null) _answers.splice(i, 1);
            else if (userIds.indexOf(_answers[i].userId) === -1) userIds.push(_answers[i].userId);
        }
        if (userIds.length > 0) {
    ...
    $scope.getUserName = function (userId) {
        if (!userId || !$scope.users) return;
        for (var i = 0; i < $scope.users.length; i++) {
            if (userId !== $scope.users[i].id) continue;
