    ...
    .directive('checkboxList', function () {
        return {
            ...
            template: '<div class="panel panel-default"> <div class="checkbox" ng-repeat="item in listItems" ' +
                'ng-show="listItems.length>0"><label><input type="checkbox"  disabled="disabled" ng-model="item.checked" ng-change="onItemChange(item)"><div class="chk-box"></div>{{item.name}}</label></div>' +
