...
angular.module('greyscaleApp')
    .directive('bulletItem', function () {
        return {
            ...
            template: '<div ng-class="{\'input-group\': !options.readonly}"><input type="text" class="form-control" ng-model="answer.data"' +
                'ng-required="!isLast && options.required" ng-readonly="options.readonly" ng-change="add()" gs-valid="answer"> ' +
                '<span class="input-group-btn" ng-hide="options.readonly"><button class="btn" ng-disabled="(options.readonly || isLast)" ng-click="remove()">' +
