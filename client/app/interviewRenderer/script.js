...
(function () {
    ...
    function readySurvey() {
        ...
        $.fetch(constUrl + 'surveys/' + surveyId,
            {method: 'GET', responseType: 'json', headers: {token: token}}).then(function (request) {
                survey = request.response;
                $('#title').innerHTML = survey.title;
                generateSurvey(survey.questions);

                $.fetch(constUrl + 'tasks/' + taskId,
                    {method: 'GET', responseType: 'json', headers: {token: token}}).then(function (request) {
...
//Generate Survey
    function generateSurvey(json) {
        ...
        for (i = 0; i < json.length; i++) {
            var type = fieldTypes[json[i].type];
            if (type) {
                dataFields.push({
                    cid: 'c' + json[i].id,
                    field_type: type,
